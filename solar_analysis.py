...
def preprocess_solar_data(file_path):
    ...
    # [중요 수정] 잘못된 위도/경도 컬럼명 교체
    # 값의 범위를 보고 자동으로 스왑하거나, 명시적으로 변경
    if df['위도'].mean() > 100: # 위도 데이터가 100이 넘으면(경도값이면)
        print("⚠️ 위도/경도 데이터가 바뀌어 있어 수정합니다.")
        df = df.rename(columns={'위도': 'temp_lon', '경도': '위도'})
        df = df.rename(columns={'temp_lon': '경도'})
