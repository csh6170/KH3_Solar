...
### 1. ⚡ 태양광 발전 예측 및 절약 금액 분석 (Solar Prediction)
...
| **Open API** | 기상청(단기예보, 지진, 태풍), 에어코리아(대기질) |
...
* **문제**: 모달 창에서 예측 수행 후 닫을 때마다 전체 페이지가 새로고침되어 기상청 API 호출 횟수가 급증하는 문제 발생.
* **해결**: `window.resetAll()` 자바스크립트 함수를 구현하여, 페이지 리로드 없이 DOM 요소만 초기화함으로써 API 트래픽을 효율적으로 관리함.

### 2. 데이터 정제 및 모델 정확도 향상
* **로직**: 태양광 발전에 유효한 시간대(06:00 ~ 20:00) 데이터를 필터링하여 모델에 주입.
* **해결**: 태양 고도에 따른 발전 효율 변화를 반영하기 위해 Bell Curve 가중치 로직을 적용하여 예측 정밀도를 높임.
...
│   └── main
│       ├── java/com/solar
│       │   ├── controller (Solar, Weather, Earthquake, Typhoon)
│       │   ├── service (Weather, TomorrowWeather, Clothing, Location)
│       │   └── dto (Data Transfer Object)
│       └── resources
│           ├── templates (index, result, earthquake, typhoon, weather)
│           ├── static (css, js, images)
│           ├── application.properties
│           └── weather_location.csv
├── predict.py           # AI 발전량 예측 엔진
├── train_model.py       # ML 모델 학습 스크립트
├── solar_analysis.py    # 데이터 분석 유틸리티
├── ai_server.py         # AI 서버 스크립트
├── convert_weather.py   # 날씨 데이터 변환
